...
export default function BillingPage() {
  ...
  const subscription = mockDb.subscriptions.find(s => s.user_id === user.id) || mockDb.subscriptions[0];
  const orders: Order[] = mockDb.orders.filter((o: Order) => o.user_id === user.id);
  const payments: Payment[] = mockDb.payments.filter((p: Payment) => p.user_id === user.id);
        ...
        <div className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-4 md:col-span-2">
          ...
          <div className="flex items-baseline gap-2">
            <h2 className="text-3xl font-extrabold text-white">
              {subscription.plan_type === 'STUDENT' ? 'Graduate Applicant Pro' : subscription.plan_type === 'PRO' ? 'Research Lab & Scholar Pro' : 'Global Explorer (Free)'}
            </h2>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-2 border-t border-slate-800/80 text-xs">
            ...
            <div>
              ...
              <span className="font-bold text-emerald-400">75 emails / mo</span>
            </div>
            <div>
